...
class UserDashboard(ctk.CTkFrame):
    ...
    def create_ui(self):
        ...
        about_btn = ctk.CTkButton(top_bar, text="ℹ️ เกี่ยวกับเรา", width=120, **btn_style, command=self.open_about)
    ...
    def load_products(self):
        ...
        # ดึงข้อมูลสินค้าจากฐานข้อมูลให้เป็น dict (เหมือน self.products เดิม)
        keys = ["id", "name", "price", "unit", "stock", "description"]
        filtered_products = [dict(zip(keys, p)) for p in products]
        ...
        for p in filtered_products:
            ...
            frame.bind("<Enter>", lambda e, f=frame: f.configure(border_width=2, border_color="#1E88E5"))
            frame.bind("<Leave>", lambda e, f=frame: f.configure(border_width=0))
            ...
            def add(pid=p['id'], stock=p['stock'], name=p['name']):
                current_qty = self.cart.get(pid, 0)
                if current_qty < stock:
                    self.cart[pid] = current_qty + 1
                    self.load_products()
                else:
                    messagebox.showwarning("สินค้าไม่เพียงพอ", f"สินค้า \"{name}\" มีในสต็อกเพียง {stock} ชิ้น")

            def remove(pid=p['id']):
                if self.cart.get(pid, 0) > 0:
                    self.cart[pid] -= 1
                    if self.cart[pid] == 0:
                        del self.cart[pid]
                    self.load_products()

            ctk.CTkButton(qty_frame, text="+", width=30, fg_color="#1E88E5", hover_color="#1565C0",
                        corner_radius=8, command=add).pack(side="left", padx=5)

            qty_var = ctk.StringVar(value=str(self.cart.get(p['id'], 0)))
            qty_entry = ctk.CTkEntry(qty_frame, textvariable=qty_var, width=40,
                                    justify="center", font=("Segoe UI", 13))
            qty_entry.pack(side="left")

            def on_qty_change(event=None, pid=p['id'], var=qty_var, stock=p['stock'], name=p['name']):
                val = var.get()
                if val.isdigit():
                    q = int(val)
                    if q > stock:
            ...
            qty_entry.bind("<Return>", on_qty_change)
            qty_entry.bind("<FocusOut>", on_qty_change)

            ctk.CTkButton(qty_frame, text="-", width=30, fg_color="#1E88E5", hover_color="#1565C0",
                        corner_radius=8, command=remove).pack(side="left", padx=5)
    ...
    def open_cart(self):
        ...
        for pid, qty in list(self.cart.items()):
            ...
            if os.path.exists(img_path):
                ...
                ctk.CTkLabel(item_frame, image=photo, text="").pack(side="left", padx=15, pady=10)
            ...
            # 🔹 ส่วนเพิ่ม/ลด/กรอกจำนวนสินค้า
            qty_frame = ctk.CTkFrame(info_frame, fg_color="#ECEFF1", corner_radius=12)
            qty_frame.pack(anchor="w", pady=8)

            def add_qty(pid=pid, stock=p['stock'], name=p['name']):
                current_qty = self.cart.get(pid, 0)
                if current_qty < stock:
                    self.cart[pid] = current_qty + 1
                    popup.destroy()
            ...
            def remove_qty(pid=pid):
                if self.cart.get(pid, 0) > 1:
                    self.cart[pid] -= 1
                else:
                    del self.cart[pid]
                popup.destroy()
            ...
            def on_qty_change(event=None, pid=pid, var=qty_var, stock=p['stock'], name=p['name']):
                val = var.get()
                if val.isdigit():
                    q = int(val)
                    if q > stock:
            ...
            ctk.CTkButton(qty_frame, text="-", width=30, height=28, fg_color="#1E88E5",
                        hover_color="#1565C0", command=remove_qty).pack(side="left", padx=5)

            qty_entry = ctk.CTkEntry(qty_frame, textvariable=qty_var, width=50, justify="center", font=("Segoe UI", 13))
            qty_entry.pack(side="left")
            qty_entry.bind("<Return>", on_qty_change)
            qty_entry.bind("<FocusOut>", on_qty_change)

            ctk.CTkButton(qty_frame, text="+", width=30, height=28, fg_color="#1E88E5",
                        hover_color="#1565C0", command=add_qty).pack(side="left", padx=5)

            # รวมราคาสินค้ารายการนี้
            ctk.CTkLabel(info_frame, text=f"รวม {total:,.2f} ฿", font=("Segoe UI", 14, "bold"),
                        text_color="#1E88E5").pack(anchor="w", pady=(5, 0))
